(function() {
 ...
 'use strict';
  ...
  function init(gen) {
    generator = gen;
    generator.addMenuItem('Tornado', 'Tornado', true, false);
  ...
  function menuClicked(e) {
    if (e.generatorMenuChanged.name === 'Tornado') {
  ...
  var createDoc = function() {
    ...
    for (var i = 0, max = items.length; i < max; i++) {
      ...
      var myFont = app.fonts.getByName("Helvetica");
      var layerRef = app.activeDocument.artLayers.add();
      layerRef.kind = LayerKind.TEXT;
      layerRef.name = i;
      layerRef.textItem.contents = "Hi!";
      var fontColor = new SolidColor();
      fontColor.rgb.red = constrain(item.color[0], 0, 255);
      fontColor.rgb.green = constrain(item.color[1], 0, 255);
      fontColor.rgb.blue = constrain(item.color[2], 0, 255);
      layerRef.textItem.color = fontColor;
      layerRef.textItem.font = myFont;
      layerRef.textItem.size = itemWidth;
